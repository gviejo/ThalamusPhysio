...
outergs = gridspec.GridSpec(2,2, figure = fig, wspace = 0.5, hspace = 0.6)
...
axE = fig.add_subplot(outergs[0,0])
...
axE.text(0.06, 1.05, "Theta spatial\n modulation", transform = axE.transAxes, fontsize = 16, multialignment='center')
#colorbar	
...
cax.set_title("Density (p < 0.01)", fontsize = 16)
...
outer = gridspec.GridSpecFromSubplotSpec(1,2, subplot_spec = outergs[1,0], wspace = 0.5, hspace = 0.5)
axF = fig.add_subplot(outer[0,0])
...
cax = inset_axes(axF, "40%", "8%",
                   bbox_to_anchor=(0.2, -0.5, 1, 1),
                   bbox_transform=axF.transAxes, 
                   loc = 'lower left')
cb = colorbar(im, cax = cax, orientation = 'horizontal', ticks = [0,1])
# cb.set_label('Density (p < 0.05)' , labelpad = -0)
cb.ax.xaxis.set_tick_params(pad = 1)
cax.set_title("Density $z_{0 ms} > $"+p_60, fontsize = 16)


# title("Ripples spatial modulation", fontsize = 7, y = 1.3)
axF.text(0.6, 1.1, "SWR spatial modulation", transform = axF.transAxes, fontsize = 16)
...
# cb.set_label('Density (p < 0.05)' , labelpad = -0)
cb.ax.xaxis.set_tick_params(pad = 1)
cax.set_title("$z_{0 ms} <$"+p_40, fontsize = 16)
...
axH = fig.add_subplot(outergs[0,1])
simpleaxis(axH)
# theta_mod, theta_ses 	= loadThetaMod('/mnt/DataGuillaume/MergedData/THETA_THAL_mod.pickle', datasets, return_index=True)
...
for i, k in enumerate(['theta', 'rip']):
	...
	axes[i].bar(np.arange(len(df))+offset, m, width = 0.4, yerr = s, label = labels[i], color = colors[i], edgecolor = 'black', linewidth = 2)
	offset +=0.4
...
axH.set_ylabel("Theta\nmodulation ($\\kappa$)", color = colors[0], labelpad = 20.0)
axH.tick_params(axis='y', colors=colors[0], pad = 1)
axHH.set_ylabel("SWR energy (|z|)", color = colors[1], labelpad = 30, rotation = -90)
...
########################################################################
axI = fig.add_subplot(outergs[1,1])
simpleaxis(gca())
...
bar(np.arange(len(tmp3)), tmp3.values, yerr = mean_score.loc[order,('score','swr','sem')], 
	linewidth = 2, color = 'none', edgecolor = 'black')
