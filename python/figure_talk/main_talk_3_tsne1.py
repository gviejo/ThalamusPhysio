...
fig.add_subplot(axD)
noaxis(axD)
sc = scatter(space[space['cluster'] == 0][1]*-1.0, space[space['cluster'] == 0][0]*-1, s = 110, c = 'white', edgecolor = 'black', alpha = 1.0, label = '_nolegend_')
# hd
scatter(space[space['cluster'] == 1][1]*-1.0, space[space['cluster'] == 1][0]*-1, s = 110, facecolor = 'white', edgecolor = 'black', alpha = 1.0, label = 'Cluster 1')
...
for i,n in enumerate(neurontoplot):
	scatter(space.loc[n,1]*-1.0, space.loc[n,0]*-1.0, s = 100, facecolor = colors[i], edgecolor = colors[i], linewidths = 2)
txts = ['a', 'b', 'c']
...
savefig(r"../../../Dropbox (Peyrache Lab)/Talks/fig_talk_6.png", dpi = 300, facecolor = 'white')
