...
# cluster data
data_klu = pd.read_hdf("../data/cluster_autocorr_waveforms.h5")
neuron_index = data_klu.index.values
mapping_position 	= pd.DataFrame(index = neuron_index, columns = ['shank', 'channel'])
###############################################################################################################
# LOADING DATA
###############################################################################################################
for m in mouses[0:1]:	
	...
	for s in sessions:		
		...
		channel 			= scipy.io.loadmat(data_directory+m+"/"+s+'/Analysis/SpikeWaveF.mat')['maxIx'][0][list(spikes.keys())]-1
		hd_info_neuron		= scipy.io.loadmat(data_directory+m+'/'+s+'/Analysis/HDCells.mat')['hdCellStats'][:,-1][list(spikes.keys())]			
		...
		for n in neurons:
			if s in n:
				idx = int(n.split("_")[1])
				mapping_position.loc[n,'shank'] = shankIndex[idx]
				mapping_position.loc[n,'channel'] = channel[idx]				
...
for m in mouses[0:1]:
	...
	cartes 		= {n:np.zeros((len(y), 8, 8)) for n in data_klu.columns}

	yn 			= np.zeros((len(y), 8))
	yn[:,0:4] 	= np.vstack(y)-np.arange(0.010, 4*0.020, 0.020)[::-1]
	yn[:,4:] 	= np.vstack(y)+np.arange(0.010, 4*0.020, 0.020)
	yn 			= yn.flatten()
	xn 			= x

	neurons 		= np.array([n for n in neuron_index if m in n])
	sessions 		= list(np.unique([n.split("_")[0] for n in neuron_index if m in n]))

	xpos 		= np.zeros(len(neurons))
	ypos 		= np.zeros(len(neurons))
	zval		= np.zeros(len(neurons))

	# for i, s in enumerate(sessions):
	# 	subset = mapping_position.filter(like=s, axis = 0)
	# 	for j in np.unique(subset['shank']):
	# 		neuron_in_shank = subset[subset['shank'] == j]
	# 		for k in np.unique(neuron_in_shank['channel']):
	# 			neuron_in_channel = neuron_in_shank[neuron_in_shank['channel'] == k]				
				
	# 			if len(neuron_in_channel) == 1:
	# 				for l in cartes.keys():
	# 					cartes[l][i, j, k] = data_klu.loc[neuron_in_channel.index,l].values+1
	# 			elif len(neuron_in_channel) > 1:
	# 				for l in cartes.keys():
	# 					tmp = data_klu.loc[neuron_in_channel.index,l].values+1
	# 				 	unique, counts = np.unique(tmp, return_counts = True)
	# 					np.argmax(counts)

	for i, n in enumerate(neurons):
		xpos[i] = x[7-mapping_position.loc[n, 'shank']]
		ypos[i] = (y[sessions.index(n.split("_")[0])] - 3*0.02 - 0.01) + mapping_position.loc[n, 'channel']*0.02
		zval[i] = data_klu.loc[n, 4]

		
	scatter(xpos, ypos, c = zval)
