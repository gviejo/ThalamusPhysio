...
simpleaxis(axA)
axA.text(-0.30, 1.01, "a", transform = axA.transAxes, fontsize = 10, fontweight='bold')
...
simpleaxis(axB)
axB.text(-0.5, 1.01, "b", transform = axB.transAxes, fontsize = 10, fontweight='bold')
...
noaxis(axC)
axC.text(-0.15, 0.95, "c", transform = axC.transAxes, fontsize = 10, fontweight='bold')
...
noaxis(axD)
axD.text(-0.2, 0.95, "d", transform = axD.transAxes, fontsize = 10, fontweight='bold')
h, w = total2.shape
...
noaxis(axE)
axE.text(-0.2, 1.0, "e", transform = axE.transAxes, fontsize = 10, fontweight='bold')
imshow(carte_adrien2, extent = bound_adrien, interpolation = 'bilinear', aspect = 'equal')
leghandles = []
...
noaxis(axF)
axF.text(-0.1, 0.95, "f", transform = axF.transAxes, fontsize = 10, fontweight='bold')
...
for i, m in enumerate(['Mouse12', 'Mouse20', 'Mouse32']):
	for j in range(4):
		...
		noaxis(ax)
		if j == 0: ax.text(-0.0, 0.95, lb[i], transform = ax.transAxes, fontsize = 10, fontweight='bold')		
		...
		if j == 0: # POSITION HEAD DIR
			...
			if i == 0: leg = legend(loc = 'lower left', fontsize = 7, frameon = False, bbox_to_anchor=(0.75, 0.1))
...
savefig("../../figures/figures_articles_v3/figart_supp_1.pdf", dpi = 900, facecolor = 'white')
os.system("evince ../../figures/figures_articles_v3/figart_supp_1.pdf &")
