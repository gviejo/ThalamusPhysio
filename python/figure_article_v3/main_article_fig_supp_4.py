...
gca().text(-0.0, 1.17, "a", transform = gca().transAxes, fontsize = 10, fontweight='bold')
...
xlim(xs[0], xs[1])
ylim(ys[0], ys[1])
text(-0.15, 1.17, 'b', transform = gca().transAxes, fontsize = 10, fontweight='bold')
title("Mouse 1")
...
for i, m in enumerate(['Mouse12', 'Mouse20', 'Mouse32']):
	...
	xlim(xs[0], xs[1])
	ylim(ys[0], ys[1])
	text(-0.15, 1.17, lbs[i], transform = gca().transAxes, fontsize = 10, fontweight='bold')
	title("Mouse "+str(i+2))
...
savefig("../../figures/figures_articles_v3/figart_supp_4.pdf", dpi = 900, facecolor = 'white')
os.system("evince ../../figures/figures_articles_v3/figart_supp_4.pdf &")
