...
for n, f in enumerate(np.sort(files)):
	...
	if n == 0:
		gca().text(-0.15, 1.1, "a", transform = gca().transAxes, fontsize = 10, fontweight='bold')
...
allradius = data2['all']['allradius']
allvelocity = data2['all']['allvelocity']

tokeep = np.logical_and(allradius.columns.values >= 10000, allradius.columns.values <= 2*1e6)

allradius = allradius.iloc[:,tokeep]
allvelocity = allvelocity.iloc[:,tokeep]
...
bounds = [10,196,374,672,2000]

ytic = np.unique(np.array(bounds).astype('int'))
ypos = [np.argmin(np.abs(allradius.columns.values/1000 - ytic[i])) for i in range(len(ytic))]

# MAPS RADIUS
subplot(gsm[0,1])
a = allradius.values.T
b = scipy.ndimage.gaussian_filter(a, (50,5))
imshow(b, aspect = 'auto', cmap = 'jet')
xticks([0, 33, 66], [-500, 0, 500])
yticks(ypos, ytic)
...
subplot(gsm[0,2])
simpleaxis(gca())
for c in allswrrad.keys():
	plot(allswrrad[c].mean(1))
ylabel("Radius (a.u.)")
xlabel("Time from SWRs (ms)")
...
# MAPS VELOCITY
subplot(gsm[0,3])
a = allvelocity.values.T
b = scipy.ndimage.gaussian_filter(a, (50,5))
imshow(b, aspect = 'auto', cmap = 'jet', vmax = 1)
xticks([0, 66], [-487.5, 487.5])
yticks(ypos, ytic)
xlabel("Time from SWRs (ms)")
ylabel("UP onset (ms)")
...
for c in allswrvel.keys():
	# plot(groups2[c], label = c[0]+'-'+c[1]+' ms')
	plot(allswrvel[c].mean(1), label = str(bounds[c-1])+'-'+str(bounds[c])+' ms')
