...
for i, lfp in zip(range(2),[lfp_rem_ex, lfp_sws_ex]):
	...
	if i == 0:
		text(-0.15, 0.9, lbs[i], transform=ax.transAxes, fontsize = 9)
		ylabel("CA1")
	else:
...
for i, n in enumerate(neurons):	
	...
	if neurons.index(n) == 0:
		ylabel("Theta cycles")
		ax.text(-0.76, 1.14, "C", transform = ax.transAxes, fontsize = 9)
	ax.spines['bottom'].set_visible(False)
	sp = phase_spike[n]
...
for i, n in enumerate(neurons):	
	...
	if neurons.index(n) == 0:
		...
		ax.text(-0.85, 1.18, "D", transform = ax.transAxes, fontsize = 9)
	if neurons.index(n) > 0:
		ax.spines['left'].set_visible(False)	
		yticks([], [])
	ax.spines['bottom'].set_visible(False)	
...
axH.text(-0.35, 1.1, "H", transform = axH.transAxes, fontsize = 9)
...
simpleaxis(gca())
gca().text(-0.3, 1.1, "I", transform = gca().transAxes, fontsize = 9)
xlabel("Nuclei")
...
savefig("../../figures/figures_articles_v2/figart_2.pdf", dpi = 900, facecolor = 'white')
os.system("evince ../../figures/figures_articles_v2/figart_2.pdf &")
