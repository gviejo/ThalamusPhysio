...
xticks([], [])
yticks([], [])
axA.text(-0.07, 1.04, "A", transform = axA.transAxes, fontsize = 10)
...
suB.text(-0.04, 1.03, "B", transform = suB.transAxes, fontsize = 10)
...
for i in range(3):
	for l,j in enumerate(['wake', 'rem', 'sws']):
		...
		if i == 0 and l == 0:
			axC[(i,l)].text(-0.8, 1.12, "C", transform = axC[(i,l)].transAxes, fontsize = 9)
...
# text(6, 24.5, 'Ventral')
# axG.invert_yaxis()
axG.text(-0.35, 1.05, "F", transform = axG.transAxes, fontsize = 10)
...
axE.text(-0.35, 1.05, 'G', transform = axE.transAxes, fontsize = 10)
...
fig.add_subplot(axH)
simpleaxis(axH)
axH.text(-0.35, 1.05, 'H', transform = axH.transAxes, fontsize = 10)
xlabel("Nuclei", labelpad = 0.1)
ylabel("Classification\n score")
...
axI.text(-0.30, 1.10, "I", transform = axI.transAxes, fontsize = 9)
...
axJ.text(-0.3, 1.10, "J", transform = axJ.transAxes, fontsize = 9)
...
axK.text(-0.3, 1.10, "K", transform = axK.transAxes, fontsize = 9)



subplots_adjust(bottom = 0.05, top = 0.97, right = 0.98, left = 0.01, hspace = 0.1)

savefig("../../figures/figures_articles_v2/figart_1.pdf", dpi = 900, facecolor = 'white')
os.system("evince ../../figures/figures_articles_v2/figart_1.pdf &")
