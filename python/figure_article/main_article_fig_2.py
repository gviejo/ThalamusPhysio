...
# TO LOAD
###############################################################################################################
data = cPickle.load(open('../figures/figures_articles/figure2/dict_fig2_article.pickle', 'rb'))
...
for i,j in zip(np.arange(0, 2*np.pi, np.pi/2),np.arange(4)):
	...
	if j == 2:		
		best_n = np.abs(allthetamodth.loc[tmp.index.values,'phase'] - (i+np.pi/8)).sort_values().index.values[5]
	elif j == 0:
...
scatter(allthetamodth.loc[best_neurons, 'phase'].values, phi2.loc[best_neurons].values.flatten(), color = 'blue', s = 5, zorder = 5)
