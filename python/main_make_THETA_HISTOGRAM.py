...
# cPickle.dump(spikes_theta_phase, open('/mnt/DataGuillaume/MergedData/SPIKE_THETA_PHASE_2.pickle', 'wb'))
# import _pickle as cPickle
# cPickle.dump(datatosave, open('/mnt/DataGuillaume/MergedData/THETA_THAL_mod.pickle', 'wb'))
# 

# COMPUTING THETA HISTOGRAM
neurons = np.unique(list(spikes_theta_phase['rem'].keys())+list(spikes_theta_phase['wake'].keys()))
bins = np.linspace(0, 2*np.pi, 20)

df = pd.DataFrame(index = bins[0:-1], columns = pd.MultiIndex.from_product([neurons, ['wak', 'rem']]))

for n in neurons:
	for ep in ['wake', 'rem']:
		tmp = spikes_theta_phase[ep][n]
		tmp += 2*np.pi
		tmp %= 2*np.pi
		a, b = np.histogram(tmp, bins, density = True)
		df[(n,ep[0:3])] = a * np.diff(b)
		

df.to_hdf("/mnt/DataGuillaume/MergedData/THETA_THAL_HISTOGRAM_2.h5", 'theta')
